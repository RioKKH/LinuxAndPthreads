...
/**
 * このプログラムは、複数のハエが画面上を飛び回る様子をシミュレートする。
 * しかし、デッドロックが発生する可能性がある。
 */
...
typedef struct {
    ...
    pthread_mutex_t mutex; /* ハエごとにミューテックスを用意する */
    /* 異なるハエの処理は並行して行うことができるようになる */
} Fly;
...
void FlyMove(Fly *fly) {
    ...
    /* FlyDistance()を呼ぶ前にアンロックを外しておく。
     * ここでfly->mutexをアンロックしてもその後に座標値の変更がないので安全である */
    pthread_mutex_unlock(&fly->mutex);
